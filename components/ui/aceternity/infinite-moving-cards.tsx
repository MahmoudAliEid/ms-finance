  ...
  return (
    <div ref={containerRef} className={cn("scroller relative z-20 max-w-7xl overflow-hidden", className)}>
      ...
      >
        {items.map((item, idx) => (
          <li
            className="w-[350px] max-w-full flex-shrink-0 rounded-2xl border border-border bg-white px-8 py-6"
            key={idx}
          >
            <div className="text-xl font-bold text-primary-gold">{item.title}</div>
